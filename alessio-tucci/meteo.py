...
def scegli_citta(risultati, citta_richiesta, parla, chiedi):
    ...
    for tentativo in range(1, 3):
        risposta_provincia = chiedi("In quale provincia si trova?")
...
# ─── METEO PRINCIPALE ─────────────────────────────────────────────────────────
def controlla_meteo(citta, parla, chiedi):
    """
    Funzione principale che orchestra geocoding, disambiguazione e reperimento
    dati meteo, formattando l'output vocale con avvisi sanitari preventivi.
    """
    print(f"\n[METEO] ════ controlla_meteo('{citta}') ════")

    # Step 1: Geocoding per cercare le coordinate della città detta dall'utente
    print("[METEO] Step 1: geocoding...")
    ...
    if not risultati:
        ...
        parla(f"Non riesco a trovare nessun posto che si chiami {citta}. "
              f"Prova a dirmi anche la provincia.")
        return

    # Step 2: Isolamento del risultato corretto tramite disambiguazione
    print("[METEO] Step 2: scelgo la città...")
    dati_citta = scegli_citta(risultati, citta, parla, chiedi)
